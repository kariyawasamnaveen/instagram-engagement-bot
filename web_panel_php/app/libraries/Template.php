...
class Template
{
	...
	function __construct($config = array())
	{
		...
		$is_cli_request = is_cli();
		if (!defined("PATH")) {
			if (!$is_cli_request && strpos(current_url(), "cron") === FALSE && get_option('enable_https', 0) == 1 && strpos($request_host, 'localhost') === FALSE) {
				if (($_SERVER["HTTPS"] ?? '') != "on") {
					define("PATH", rtrim(base_url(), '/') . "/index.php/");
					if (strpos(base_url(), "https") === FALSE) {
						redirect(str_replace("http://", "https://", base_url()));
					}
				} else {
					define("PATH", str_replace("http://", "https://", rtrim(base_url(), '/')) . "/index.php/");
				}
			} else {
				define("PATH", rtrim(base_url(), '/') . "/index.php/");
			}
