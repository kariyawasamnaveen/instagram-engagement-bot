        ...
        switch ($controller_name) {
            ...
            case 'dripfeed':
                ...
                $order_attrs = [
                    ...
                    'total_charge'   => (float) $item['charge'],
                    'runs'           => $runs,
            ...
            default:
                $charge = get_option("currency_symbol", "") . (float) $item['charge'];
                if ($item['formal_charge'] != 0 && staff_has_permission($controller_name, 'see_cost')) {
                    $charge .= ' (' . (float) $item['formal_charge'] . ' / <span class="text-info">' . (float) $item['profit'] . '</span>)';
                }
                ...
                if (!empty($item['delivery_speed'])) {
                    $order_attrs['delivery_speed'] = ucfirst(str_replace('_', ' ', $item['delivery_speed']));
                }

                if (!empty($item['custom_tags'])) {
                    $order_attrs['targeting'] = ucfirst(str_replace(',', ', ', $item['custom_tags']));
                }
                break;
        }
        // Permission
        ...
        switch ($controller_name) {
            ...
            case 'dripfeed':
                ...
                $order_attrs = [
                    ...
                    'total charge'   => ['name' => lang('Amount'), 'value' => (float) $item['charge']],
            ...
            default:
                ...
                if (!empty($item['delivery_speed'])) {
                    $order_attrs['delivery_speed'] = ['name' => 'Delivery Speed', 'value' => ucfirst(str_replace('_', ' ', $item['delivery_speed']))];
                }

                if (!empty($item['custom_tags'])) {
                    $order_attrs['targeting'] = ['name' => 'Targeting', 'value' => ucfirst(str_replace(',', ', ', $item['custom_tags']))];
                }
                break;
        }
        $xhtml_order_attr = null;
...
if (! function_exists('show_item_rate')) {
    ...
    {
        $rate = (float) $item_service['price'];
        if ($task == 'user' && session('uid')) {
            $rate = (array_key_exists($item_service['id'], $items_custom_rate)) ? (float) $items_custom_rate[$item_service['id']] : (float) $item_service['price'];
        }
        return $rate;
    }
}
