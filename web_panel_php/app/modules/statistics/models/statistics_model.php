...
class statistics_model extends MY_Model
{
    ...
    public function header_statistics()
    {
        ...
        // --- NEW CODE: AI Engine Live Data ---
        $uid = session('uid');
        $today = date('Y-m-d');
        
        // Get VIP Follows today
        ...
        $this->db->where('DATE(o.created)', $today);
        $this->db->group_start();
        $this->db->like('s.name', 'VIP');
        $this->db->like('s.name', 'Follow');
        $this->db->group_end();
        $query_f = $this->db->get();
        $ai_followers_today = $query_f->row()->quantity ?? 0;
        ...
        $this->db->group_start();
        $this->db->like('s.name', 'VIP');
        $this->db->like('s.name', 'Like');
        $this->db->group_end();
        $query_l = $this->db->get();
        $ai_likes_today = $query_l->row()->quantity ?? 0;

        // Get active campaigns count (Only VIP)
        $get_active = function($keyword) use ($uid) {
            $this->db->select('o.id');
            $this->db->from($this->tb_order . ' o');
            $this->db->join($this->tb_services . ' s', 's.id = o.service_id', 'left');
            $this->db->where('o.uid', $uid);
            $this->db->where_in('o.status', ['processing', 'inprogress', 'pending']);
            
            $this->db->group_start();
            if ($keyword == 'Viral') {
                $this->db->like('s.name', 'Viral');
                $this->db->or_like('s.name', 'Multiplier');
                $this->db->or_like('s.name', 'Repost');
            } else {
                $this->db->like('s.name', 'VIP');
        ...
        $ai_active_likes = $get_active('Like');
        $ai_active_follows = $get_active('Follow');
        $ai_active_comments = $get_active('Comment');
        $ai_active_viral = $get_active('Viral');
        // --- END NEW CODE ---
        ...
        $result = [
            ...
            'tickets' => [
                ...
                'icon' => 'fa fa-ticket'],
            'ai_stats' => [
                'followers_today' => $ai_followers_today,
                'likes_today' => $ai_likes_today,
                'active_likes' => $ai_active_likes,
                'active_follows' => $ai_active_follows,
                'active_comments' => $ai_active_comments,
                'active_viral' => $ai_active_viral
            ]
        ];
        return $result;
    }
