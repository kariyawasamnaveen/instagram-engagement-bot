        ...
        <div>
          <?php foreach ($cron_links as $key => $item_link): ?>
            <div class="item m-t-20">
              <div><strong><?=esc(ucwords(str_replace('_', ' ', $key))); ?></strong></div>
              <code><?=esc(sprintf($link_format, $item_link)); ?></code>
            </div>
          <?php endforeach ?>
        </div>
        <div class="note m-t-30">
          Use <strong>Automation Dry Run</strong> only for manual testing. It simulates local automation without sending real Instagram actions.
        </div>
        <div class="m-t-30">
