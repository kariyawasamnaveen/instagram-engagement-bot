...
class api_model extends MY_Model
{
    ...
    public function get_services_list_old_version($uid = "")
    {
        $data = array();
    ...
    public function get_services_list($uid = "")
    {
        ...
        $this->db->select('
            s.id as service, s.name, c.name as category, COALESCE(gup.service_price, 
            s.price) AS rate, s.min, s.max, s.type, s.desc, s.dripfeed, s.refill
        ');
    ...
    public function get_item($params = null, $option = null)
    {
        ...
        if ($option['task'] == 'check-duplicate-order') {
            ...
            $query = $this->db->get($this->tb_orders);
            if ($query->num_rows() > 0) {
                return true;
            } else {
                return false;
