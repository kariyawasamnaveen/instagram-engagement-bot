...
async def check_subscription(p, username):
    ...
    cursor.execute("SELECT * FROM ig_accounts WHERE status = 1 AND cookies IS NOT NULL AND username NOT LIKE '+91%' AND username NOT REGEXP '^91[0-9]' ORDER BY RAND() LIMIT 5")
...
async def perform_action(p, acc, task, cursor, db):
    ...
    try:
        ...
        if "airproxy" not in PROXY_SETTINGS['server']:
            proxy_url = f"http://{proxy_username}:{proxy_password}@YOUR_SERVER_IP:30909"
        else:
            base_server = PROXY_SETTINGS['server'].replace('http://', '')
            proxy_url = f"http://{proxy_username}:{proxy_password}@{base_server}"
        ...
        if action == 'follow':
            ...
            following_exact = re.compile(r"^(Requested|Following|Richiesta effettuata|Inviata|Segui já|Siguiendo|Pendiente|Mengikuti|Abonné\(e\)|Abonnements|Folge ich|Gefolgt|Takipdesin|İstek Gönderildi|Вы подписаны|Запрос отправлен)$", re.IGNORECASE)
            ...
            await page.screenshot(path="/var/www/html/app/logs/fail.png"); logging.info(f"Like btn not found. URL: {page.url}, Title: {await page.title()}")
        ...
        if context:
            try:
                ...
                passbug_log(f"COOKIES EXPIRED (Redirect Loop) for Account {acc.get('username')}! Marking as Needs Relogin.")
            return False, "COOKIES_EXPIRED"
        return False, err_msg
...
async def worker_loop(p):
    ...
    while True:
        try:
            ...
            cursor.execute("SELECT * FROM automation_tasks WHERE status = 0 AND action IN ('comment', 'share', 'repost') AND (run_after IS NULL OR run_after <= NOW()) ORDER BY id ASC LIMIT 1 FOR UPDATE SKIP LOCKED")
            ...
            if task:
                ...
                cursor.execute("SELECT * FROM ig_accounts WHERE status = 1 AND username NOT LIKE '+91%' AND username NOT REGEXP '^91[0-9]' ORDER BY last_action_at ASC LIMIT 1 FOR UPDATE SKIP LOCKED")
                acc = cursor.fetchone()
                ...
                if acc:
                    ...
                    try:
                        ...
                        if msg in ("ACCOUNT_BLOCKED", "ACTION_BLOCKED_SILENT", "ACTION_BLOCKED_API"):
                            if "ACTION_BLOCKED" in msg:
                                debug_log(f"SOFT BLOCK: Account {acc['username']} hit Action Block ({msg}). Resting for 48h.")
                                cursor.execute("UPDATE ig_accounts SET status = 5, cooldown_until = NOW() + INTERVAL 48 HOUR WHERE id = %s", (acc['id'],))
                                logging.warning(f"Account {acc['username']} SOFT BLOCKED ({msg}). Resting for 48h.")
                            else:
                                debug_log(f"ACCOUNT ERROR: Account {acc['username']} HARD BLOCKED/CHECKPOINT ({msg}). Switching account.")
                    ...
                    # Unified check for order completion/cancellation
                ...
                if random.random() < 0.1:
                    cursor.execute("SELECT * FROM ig_accounts WHERE status = 1 AND username NOT LIKE '+91%' AND username NOT REGEXP '^91[0-9]' ORDER BY RAND() LIMIT 1")
