        ...
        for task in bundle_tasks:
            ...
            if action == 'follow':
                ...
                if await page.locator('button, div[role="button"], a[role="button"]').filter(has_text=following_pattern).first.is_visible(timeout=5000):
                    ...
                    bundle_results.append((task, True, "Already followed/requested")); continue
                ...
                if not await follow_btn.is_visible(timeout=5000):
                    if await page.locator('button, div[role="button"], a[role="button"]').filter(has_text=following_pattern).first.is_visible(timeout=5000):
                        ...
                        bundle_results.append((task, True, "Already followed/requested")); continue
                    pass # context closed later
                    bundle_results.append((task, False, "Follow button not found")); continue
                ...
                if api_blocked:
                    ...
                    pass # context closed later
                    bundle_results.append((task, False, "ACTION_BLOCKED_API")); continue
                ...
                if await page.get_by_role("button", name=following_exact, exact=True).first.is_visible(timeout=8000):
                    ...
                    print(success_msg)
                    pass # context closed later
                    bundle_results.append((task, True, "Followed successfully")); continue
                else:
                    debug_log("DOM Validation detected Action Block (Ghosted Follow)!")
                    pass # context closed later
                    bundle_results.append((task, False, "ACTION_BLOCKED_SILENT")); continue
                ...
                if await unlike_btn.is_visible(timeout=5000):
                    pass # context closed later
                    bundle_results.append((task, True, "Already liked")); continue
                ...
                if await like_btn.is_visible(timeout=5000):
                    ...
                    if await unlike_btn_after.is_visible(timeout=5000):
                        ...
                        print(success_msg)
                        pass # context closed later
                        bundle_results.append((task, True, "Liked successfully")); continue
                    else:
                ...
                pass # context closed later
                bundle_results.append((task, False, "Like button not found")); continue
                ...
                if share_btn_el:
                    ...
                    try:
                        ...
                        pass # context closed later
                        bundle_results.append((task, False, f"Share button click failed: {click_e}")); continue
                            ...
                            if clicked:
                                ...
                                for sel in send_btn_selectors:
                                    try:
                                        ...
                                        print(success_msg)
                                        pass # context closed later
                                        bundle_results.append((task, True, f"{action.capitalize()}ed successfully via DM")); continue
                                    except:
                                        continue
                    ...
                    if not dm_sent:
                        ...
                        pass # context closed later
                        bundle_results.append((task, False, "Share DM failed - Copy Link does not count as real share")); continue
                else:
                    debug_log(f"Share button not found for target: {target}. All selectors exhausted.")
                    bundle_results.append((task, False, "Share button not found")); continue
                ...
                if not comment_input:
                    ...
                    pass # context closed later
                    bundle_results.append((task, False, "Comment input field not found")); continue
                ...
                try:
                    ...
                    await save_error_screenshot(page, acc, action, task['id'])
                    pass # context closed later
                    bundle_results.append((task, False, "Failed to type comment in input field")); continue
                ...
                if input_cleared:
                    ...
                    print(success_msg)
                    pass # context closed later
                    bundle_results.append((task, True, "Comment placed successfully")); continue
                else:
                    debug_log(f"Re-verification Warning: Input box is still full. The comment might not have posted correctly.")
                    await save_error_screenshot(page, acc, action, task['id'])
                    pass # context closed later
                    bundle_results.append((task, False, "Comment was typed but not submitted (Input still contains text)")); continue
...
async def worker_loop(p):
    ...
    while True:
        try:
            ...
            cursor.execute("SELECT target FROM automation_tasks WHERE status = 0 AND (run_after IS NULL OR run_after <= NOW()) ORDER BY id ASC LIMIT 1 FOR UPDATE SKIP LOCKED")
