...
import requests
import time
PROXY_SETTINGS = {'server': 'http://YOUR_AIRPROXY_SERVER:30909', 'username': 'YOUR_AIRPROXY_USER', 'password': 'YOUR_AIRPROXY_PASS_2'}
...
async def perform_ghost_activity(p, acc, cursor, db):
    debug_log(f"Starting Ghost Activity (Warm-up) for {acc['username']}")
...
async def perform_action(p, acc, task, cursor, db):
    ...
    try:
        ...
        proxy_url = f"http://{PROXY_SETTINGS['username']}:{PROXY_SETTINGS['password']}@YOUR_SERVER_IP:30909" if "airproxy" not in PROXY_SETTINGS['server'] else f"http://{PROXY_SETTINGS['username']}:{PROXY_SETTINGS['password']}@YOUR_AIRPROXY_SERVER:30909"
            ...
            if task:
                ...
                if warmup_acc:
                    if last_account_id != warmup_acc['id']:
                        logging.info(f"Account changed to {warmup_acc['username']} for WARMUP. Rotating AirProxy IP...")
                        try:
                            requests.get("https://airproxy.io/api/proxy/change_ip/?format=json&id=1834&key=YOUR_AIRPROXY_API_KEY", timeout=15)
                            time.sleep(25)
                        except Exception as e:
